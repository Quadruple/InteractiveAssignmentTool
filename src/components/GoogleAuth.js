...
class GoogleAuth extends React.Component {
  componentDidMount() {
    window.gapi.load("client:auth2", () => {
      window.gapi.client
        .init({
          clientId:
            "976909452236-aesh2ots06cpm2a5jjubr4b47sn19ftv.apps.googleusercontent.com",
          scope: "email",
        })
        .then(() => {
  ...
  onAuthChange = (isSignedIn) => {
    if (isSignedIn) {
      ...
      const userMail = this.auth.currentUser.get().nt.Wt;
  ...
  renderAuthButton() {
    if (this.props.isSignedIn === null && false) {
      return null;
